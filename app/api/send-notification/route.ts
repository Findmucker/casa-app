...
export async function POST(request: Request) {
  try {
    ...
    if (!target?.uid) {
      return NextResponse.json({ error: "Target is not in your house" }, { status: 403 });
    }

    let tokenDoc = await db.collection("fcm_tokens").doc(target.uid).get();
    if (!tokenDoc.exists) {
      // Transitional fallback for tokens registered before they were keyed by UID.
      tokenDoc = await db.collection("fcm_tokens").doc(normalizedTarget).get();
    }
